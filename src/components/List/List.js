...
const List = () => {
  const {books_data,deleteBook} = useContext(BookContext);
  console.log("data from list: ",books_data);
  return(
    <ul>
      {
        books_data.map( book => <li><Book name = {book.name} image = {book.image}/><div  style = {style}><CRUD deleteBook = {deleteBook} book = {book}/></div></li>)
      }
    </ul>
